...
path ='C:/Users/user/Desktop/뉴스/signal_5stock'
...
for i in range(0,5):
    for e in df[i].index:
        try:
            if df[i]['signal'][e]+df[i]['signal'][e+1]==0:
                df[i]['position'][e+1]='no action'
            elif df[i]['signal'][e]+df[i]['signal'][e+1]==2:
                df[i]['position'][e+1]='holding'
            elif df[i]['signal'][e] > df[i]['signal'][e+1]:
...
result.to_csv('C:/Users/user/Desktop/뉴스/signal_5stock/5stock_result.csv')
